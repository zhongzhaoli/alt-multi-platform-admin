<template>
  <div ref="tagsViewRef" class="tagsViewContainer">
    <ScrollPane :tag-refs="tagRefs">
      <router-link
      ...
      >
        ...
        <div v-if="!isAffix(item)" class="close" @click.prevent.stop="closeTag(item)">
...
const closeOthersTags = () => {
  ...
  if (selectedPath !== route.path && selectedPath !== undefined) router.push(selectedPath);
  closeMenu();
};
...
const init = () => {
  affixTags = filterAffixTags(unref(routerStore.allRoutes) as RouteRecordRaw[]);
...
onMounted(() => {
  ...
  addRouteListener((route) => {
    if (route.path === PageEnum.BASE_LOGIN) return;
    addTags(route);
  }, true);
});
</script>
