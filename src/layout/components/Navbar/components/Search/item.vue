<template>
  <div class="searchItemComponent" :class="{ active: active }" @mouseenter="mouseEnter">
    <div class="textBox">
      <div v-for="(v, i) in item.list" :key="i">
        <i v-if="v.icon" class="icon" :class="v.icon" />
        <div class="title">
          {{ v.title }}
        </div>
        <div v-if="index < item.list.length - 1" class="right">
