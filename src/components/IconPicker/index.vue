<template>
  <el-popover trigger="click" width="400">
    <template #reference>
      <el-input v-model="formValue" readonly style="width: 100%" placeholder="请选择图标">
        <template v-if="formValue" #prepend>
          <i :class="formValue" />
        </template>
      </el-input>
    </template>
    <template #default>
      <el-row :gutter="10">
        <el-col :span="20" style="margin-bottom: 10px">
          <el-input v-model="keyWord" placeholder="查询想要的图标" @change="searchFun" />
        </el-col>
        <el-col :span="4">
          <el-button type="primary" style="width: 100%" @click="searchFun"> 查询 </el-button>
        </el-col>
      </el-row>
      <el-row :gutter="10" class="mt-10">
        <el-col v-for="item in showList" :key="item" :span="6">
      ...
      <div class="d-flex">
        ...
        />
        <el-button v-if="formValue" style="margin-left: 14px" @click="clear"> 清空 </el-button>
      </div>
    </template>
  </el-popover>
</template>
...
const searchFun = () => {
  const searchList = iconList.value.filter((icon) => icon.includes(keyWord.value));
